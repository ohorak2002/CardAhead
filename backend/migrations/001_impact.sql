...
begin;
create schema if not exists cardahead_private;
revoke all on schema cardahead_private from public, anon, authenticated;
create table cardahead_private.owner_config (
  singleton boolean primary key default true check (singleton),
  user_id uuid not null references auth.users(id)
);
-- Intentionally no owner seed. A trusted operator provisions one verified auth.users.id.
create table cardahead_private.participants (
  user_id uuid primary key references auth.users(id) on delete cascade,
  enabled boolean not null default false,
  epoch uuid not null,
  updated_at timestamptz not null default now()
);
create table cardahead_private.impact (
...
create unique index impact_recommendation_kind on cardahead_private.impact(user_id,recommendation_id,kind) where recommendation_id is not null;
alter table cardahead_private.owner_config enable row level security;
alter table cardahead_private.participants enable row level security;
alter table cardahead_private.impact enable row level security;
-- No table policies or grants: even the owner account uses guarded aggregate functions.
revoke all on all tables in schema cardahead_private from public, anon, authenticated;

create function public.cardahead_is_owner() returns boolean language sql stable security definer set search_path = '' as $$
  select exists(select 1 from cardahead_private.owner_config c join auth.users u on u.id = c.user_id
    where c.singleton and u.id = auth.uid() and u.email_confirmed_at is not null);
$$;

create function public.cardahead_set_sharing(enabled boolean, epoch uuid) returns void
...
begin
  ...
  insert into cardahead_private.participants(user_id,enabled,epoch) values(auth.uid(),enabled,epoch)
  on conflict(user_id) do update set enabled = excluded.enabled, epoch = excluded.epoch, updated_at = now();
end;
$$;

create function public.cardahead_upload(epoch uuid, records jsonb) returns void
language plpgsql security definer set search_path = '' as $$
declare r jsonb; consent cardahead_private.participants;
begin
  if auth.uid() is null then raise exception 'Authentication required' using errcode = '42501'; end if;
  select * into consent from cardahead_private.participants where user_id = auth.uid() for update;
  ...
  for r in select * from jsonb_array_elements(records) loop
    ...
    insert into cardahead_private.impact values(auth.uid(),(r->>'id')::uuid,(r->>'recommendation_id')::uuid,
...
create function public.cardahead_delete_shared() returns void language plpgsql security definer set search_path = '' as $$
begin
  if auth.uid() is null then raise exception 'Authentication required' using errcode = '42501'; end if;
  -- Lock the same row as upload: no in-flight upload can resurrect deleted records.
  update cardahead_private.participants set enabled = false, epoch = gen_random_uuid(), updated_at = now() where user_id = auth.uid();
  delete from cardahead_private.impact where user_id = auth.uid();
end;
$$;

create function public.cardahead_dashboard(from_day date, to_day date) returns jsonb
language plpgsql stable security definer set search_path = '' as $$
declare result jsonb;
begin
  if not public.cardahead_is_owner() then raise exception 'Owner only' using errcode = '42501'; end if;
  ...
  with data as (select * from cardahead_private.impact where day between from_day and to_day),
  groups as (
  ...
  select jsonb_build_object(
    'participating_users',(select count(*) from cardahead_private.participants where enabled),
...
$$;
revoke all on function public.cardahead_is_owner(), public.cardahead_set_sharing(boolean,uuid), public.cardahead_upload(uuid,jsonb), public.cardahead_delete_shared(), public.cardahead_dashboard(date,date) from public, anon;
grant execute on function public.cardahead_is_owner(), public.cardahead_set_sharing(boolean,uuid), public.cardahead_upload(uuid,jsonb), public.cardahead_delete_shared(), public.cardahead_dashboard(date,date) to authenticated;
commit;
